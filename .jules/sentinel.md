...
## 2026-02-24 - Unauthenticated Local API Exposure
**Vulnerability:** The internal `go2rtc` camera streaming service API was listening on all interfaces (`0.0.0.0`) without any authentication configuration. This exposed full control of the streaming service (adding/removing streams) and the video feeds themselves to anyone on the local network, bypassing the application's WebUI authentication layer.
**Learning:** Embedding third-party binaries often introduces parallel security contexts. If the main app is secure but spawns a helper service on a separate port without explicit security configuration, the helper service becomes a backdoor.
**Prevention:** Always explicitly configure authentication for helper services/binaries. For services requiring local access (like video players), generate strong random credentials at runtime and inject them into the service configuration and the client connection strings.
