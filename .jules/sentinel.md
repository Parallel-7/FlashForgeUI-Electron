...
## 2026-02-20 - Unauthenticated Go2rtc API Access
**Vulnerability:** The `go2rtc` streaming server was listening on port 1984 on all interfaces (`0.0.0.0`) without authentication. This allowed anyone on the local network to view camera streams, add/remove streams (SSRF risk), and potentially execute commands.
**Learning:** Embedded services running alongside the main application often default to insecure "dev-friendly" configurations (like binding to `0.0.0.0` with no auth). When integrating third-party binaries, always audit their default security posture.
**Prevention:** Explicitly configure authentication for all bundled services. Generate random credentials at runtime if the service doesn't support integration with the main app's auth system directly. Use `token` parameters for WebSocket authentication when HTTP headers are not an option.
