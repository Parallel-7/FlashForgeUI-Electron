...
Generated: 2026-07-09T16:27:42.108Z
Total files scanned: 381
Files with @fileoverview: 372
...
## src/main/ipc/WindowControlHandlers.ts
...
Camera IPC handler for managing camera streaming operations via go2rtc.
...
## src/main/ipc/handlers/__tests__/printer-power-handlers.test.ts

Unit tests for the remote reboot handler's pure logic.
...
## src/main/ipc/handlers/backend-handlers.ts
...
## src/main/ipc/handlers/calibration-handlers.ts

IPC handlers for calibration operations.
Exposes calibration functionality to renderer processes through IPC.

@module main/ipc/handlers/calibration-handlers

## src/main/ipc/handlers/camera-handlers.ts
...
## src/main/ipc/handlers/component-dialog-handlers.ts
...
## src/main/ipc/handlers/connection-handlers.ts
...
## src/main/ipc/handlers/control-handlers.ts
...
## src/main/ipc/handlers/e2e-handlers.ts

Guarded IPC hooks used exclusively by live hardware E2E coverage.
...
## src/main/ipc/handlers/file-manager-handlers.ts

IPC handlers for the SFTP-based printer file manager dialog.
...
## src/main/ipc/handlers/job-handlers.ts
...
## src/main/ipc/handlers/material-handlers.ts
...
## src/main/ipc/handlers/palette-handlers.ts

IPC handlers for Component Palette window
...
## src/main/ipc/handlers/printer-power-handlers.ts

IPC handler for the remote "Reboot Printer" feature (desktop only).
...
Lifecycle (driven over the 'printer:reboot-status' push channel):
 ...
 4. success      - emitted when polling resumes (printer is back online).
...
## src/main/ipc/handlers/spoolman-handlers.ts
...
## src/main/ipc/handlers/ssh-settings-handlers.ts

IPC handlers for the centralized per-printer SSH settings.
...
## src/main/ipc/handlers/update-handlers.ts
...
## src/main/ipc/handlers/webui-handlers.ts
...
## src/main/ipc/printer-context-handlers.ts
...
## src/main/managers/CalibrationManager.ts

Central manager for calibration operations.
...
## src/main/managers/HeadlessManager.ts
...
## src/main/managers/LoadingManager.ts
...
## src/main/managers/PrinterBackendManager.ts
...
## src/main/managers/PrinterContextManager.ts
...
## src/main/managers/__tests__/PrinterBackendManager.test.ts

Tests for PrinterBackendManager backend selection, lifecycle wiring,
and context-driven backend reuse behavior.

## src/main/managers/__tests__/PrinterContextManager.test.ts

Tests for PrinterContextManager context creation, activation,
...
## src/main/printer-backends/Creator5Backend.ts

Backend implementation for Creator 5 / Creator 5 Pro printers.
...
## src/main/printer-backends/DualAPIBackend.ts
...
## src/main/printer-backends/MaterialStationBackend.ts

Shared base for material-station printers (AD5X and Creator 5 / 5 Pro).
...
Tests for BasePrinterBackend feature normalization, status helpers,
and default backend behavior shared by concrete printer implementations.

## src/main/printer-backends/ad5x/ad5x-transforms.ts
...
## src/main/printer-backends/ad5x/ad5x-types.ts
...
## src/main/printer-backends/ad5x/ad5x-utils.ts
...
## src/main/printer-backends/ad5x/index.ts
...
## src/main/services/AutoConnectService.ts
...
Auto launch service managing OS login-item registration for start-at-boot and start-minimized behavior.
...
Shared camera stream reconciliation helpers for desktop IPC and WebUI routes.

## src/main/services/ConnectionEstablishmentService.ts
...
## src/main/services/ConnectionStateManager.ts
...
## src/main/services/ContextServiceInitializer.ts

Shared per-context service initialization for polling-driven runtime services.
...
## src/main/services/DebugLogService.ts

Debug logging service for file-based log persistence
...
## src/main/services/DialogIntegrationService.ts
...
## src/main/services/EnvironmentDetectionService.ts
...
SFTP-backed file management service for FlashForge printers.
...
## src/main/services/Go2rtcBinaryManager.ts

Manages the go2rtc binary lifecycle including platform detection,
...
## src/main/services/Go2rtcService.ts

Unified camera streaming service using go2rtc as the streaming gateway.
...
## src/main/services/MainProcessPollingCoordinator.ts
...
## src/main/services/MultiContextNotificationCoordinator.ts
...
## src/main/services/MultiContextPollingCoordinator.ts
...
## src/main/services/MultiContextPrintStateMonitor.ts
...
## src/main/services/MultiContextSpoolmanTracker.ts
...
## src/main/services/MultiContextTemperatureMonitor.ts
...
## src/main/services/PrintStateMonitor.ts
...
## src/main/services/PrinterDataTransformer.ts
...
## src/main/services/PrinterDiscoveryService.ts
...
## src/main/services/PrinterPollingService.ts
...
## src/main/services/SSHSettingsService.ts

Centralized per-printer SSH credential store.

Single source of truth for SSH credentials used by every SSH-based feature
(file manager, calibration assistant desktop dialog, WebUI calibration
routes). Settings are keyed by printer serial number — stable across
sessions, unlike runtime context ids — and persisted to ssh-settings.json in
the Electron userData directory with passwords encrypted via SecureStorage
(Electron safeStorage with base64 fallback).
...
## src/main/services/SavedPrinterService.ts
...
## src/main/services/SpoolmanHealthMonitor.ts
...
## src/main/services/SpoolmanIntegrationService.ts
...
## src/main/services/SpoolmanService.ts
...
## src/main/services/SpoolmanUsageTracker.ts
...
## src/main/services/TemperatureMonitoringService.ts
...
## src/main/services/ThumbnailCacheService.ts
...
## src/main/services/ThumbnailRequestQueue.ts
...
## src/main/services/__tests__/AutoConnectService.test.ts

Tests for AutoConnectService decision-making across single-match,
multi-match, and no-match saved-printer scenarios.

## src/main/services/__tests__/AutoLaunchService.test.ts

Unit tests for AutoLaunchService
...
## src/main/services/__tests__/ConnectionEstablishmentService.test.ts

Tests for ConnectionEstablishmentService client selection,
initialization flow, and printer-info handshake handling.

## src/main/services/__tests__/ConnectionStateManager.test.ts

Tests for ConnectionStateManager state transitions, client cleanup,
and PrinterContextManager updates during connection changes.

## src/main/services/__tests__/EnvironmentDetectionService.test.ts
...
## src/main/services/__tests__/Go2rtcService.test.ts

Jest coverage for Go2rtcService stream reconciliation behavior.

## src/main/services/__tests__/MultiContextPollingCoordinator.test.ts

Tests for MultiContextPollingCoordinator polling service creation,
rebinding, disposal, and event fan-out across printer contexts.

## src/main/services/__tests__/PrinterDataTransformer.test.ts

Tests for PrinterDataTransformer normalization of backend status,
job metadata, and legacy field compatibility.

## src/main/services/__tests__/PrinterDiscoveryService.test.ts

Tests for PrinterDiscoveryService discovery orchestration,
singleton lifecycle, and ff-api adapter behavior.

## src/main/services/__tests__/SSHSettingsService.test.ts

Unit tests for SSHSettingsService.
...
## src/main/services/__tests__/SavedPrinterService.test.ts

Tests for SavedPrinterService persistence helpers delegated to
PrinterDetailsManager for saved-printer CRUD and last-used selection.

## src/main/services/__tests__/StaticFileManager.test.ts
...
## src/main/services/calibration/__tests__/Bed.test.ts

Unit tests for the Bed model class.
Tests bed mesh data management, corner calculations, and statistics.

@module main/services/calibration/__tests__/Bed.test

## src/main/services/calibration/__tests__/DeviationAnalyzer.test.ts

Unit tests for the DeviationAnalyzer class.
...
## src/main/services/calibration/__tests__/FFTProcessor.test.ts

Unit tests for the FFTProcessor class.
...
## src/main/services/calibration/__tests__/KlipperConfigParser.test.ts

Unit tests for the KlipperConfigParser class.
...
## src/main/services/calibration/__tests__/ScrewSolver.test.ts

Unit tests for the ScrewSolver class.
...
## src/main/services/calibration/__tests__/ShaperAnalyzer.test.ts

Unit tests for the ShaperAnalyzer class.
...
## src/main/services/calibration/__tests__/TapeCalculator.test.ts

Unit tests for the TapeCalculator class.
Tests tape compensation calculations for bed leveling.

@module main/services/calibration/__tests__/TapeCalculator.test

## src/main/services/calibration/__tests__/WorkflowEngine.test.ts
...
## src/main/services/calibration/engine/Bed.ts

Model representing the 3D printer bed with mesh data management.
...
## src/main/services/calibration/engine/DeviationAnalyzer.ts

Analyzer for bed mesh deviations and calibration strategy determination.
...
## src/main/services/calibration/engine/ScrewSolver.ts

Calculator for bed leveling screw adjustments.
...
## src/main/services/calibration/engine/TapeCalculator.ts

Calculator for tape compensation recommendations.
...
## src/main/services/calibration/engine/WorkflowEngine.ts

Multi-stage calibration workflow orchestration engine.
...
## src/main/services/calibration/engine/index.ts

Exports for the calibration engine module.
...
## src/main/services/calibration/index.ts

Main exports for the calibration module.
...
## src/main/services/calibration/parsers/KlipperConfigParser.ts

Parser for Klipper printer.cfg files to extract bed mesh data.
...
## src/main/services/calibration/report/ReportRenderer.ts

Utilities for rendering calibration reports to binary formats.
...
## src/main/services/calibration/shaper/FFTProcessor.ts

Fast Fourier Transform processor for accelerometer data.
...
## src/main/services/calibration/shaper/ShaperAnalyzer.ts

Input shaper analyzer for Klipper resonance compensation.
...
## src/main/services/calibration/shaper/index.ts

Exports for the input shaper calibration module.
Provides FFT processing and shaper analysis for resonance compensation.

@module main/services/calibration/shaper

## src/main/services/calibration/ssh/SCPFileTransfer.ts

SCP file transfer for printer configuration and calibration files.
...
## src/main/services/calibration/ssh/SSHConnectionManager.ts

SSH connection management for remote printer access.
...
## src/main/services/calibration/ssh/index.ts

Exports for the SSH module.
Provides SSH connectivity and file transfer functionality for remote printer access.

@module main/services/calibration/ssh

## src/main/services/discord/DiscordNotificationService.ts
...
## src/main/services/discord/__tests__/DiscordNotificationService.test.ts

Focused unit tests for DiscordNotificationService timer and lifecycle behavior.
...
## src/main/services/discord/index.ts
...
## src/main/services/notifications/NotificationService.ts
...
## src/main/services/notifications/PrinterNotificationCoordinator.ts
...
## src/main/types/go2rtc.types.ts

Type definitions for go2rtc REST API and service integration.
...
## src/main/utils/CSSVariables.ts
...
## src/main/utils/EventEmitter.ts
...
## src/main/utils/HeadlessArguments.ts
...
## src/main/utils/HeadlessDetection.ts
...
## src/main/utils/HeadlessLogger.ts
...
## src/main/utils/PrinterUtils.ts
...
## src/main/utils/RoundedUICompatibility.ts
...
## src/main/utils/SecureStorage.ts

Lightweight helpers for encrypting/decrypting secrets at rest.
...
## src/main/utils/__tests__/PrinterUtils.test.ts

Tests for product-ID-first printer family/model detection.
...
Tests for camera utility helpers covering stream-type detection,
...
## src/main/utils/extraction.utils.ts
...
## src/main/webui/schemas/__tests__/web-api.schemas.test.ts

Tests for WebUI Zod schemas that validate job-start payloads
and theme profile operations.

## src/main/webui/schemas/web-api.schemas.ts
...
## src/main/webui/server/AuthManager.ts
...
## src/main/webui/server/CameraStreamProxy.ts

Authenticated WebSocket proxy tunneling WebUI camera streams to go2rtc.
...
## src/main/webui/server/WebSocketManager.ts
...
## src/main/webui/server/WebUIManager.ts
...
## src/main/webui/server/__tests__/AuthManager.test.ts

Tests for AuthManager covering login validation, token verification,
and timing-attack resistance in password/HMAC comparisons.

## src/main/webui/server/__tests__/CameraStreamProxy.test.ts

Jest coverage for `CameraStreamProxy`.
...
## src/main/webui/server/__tests__/WebSocketManager.test.ts

Tests for WebSocketManager authentication, message handling,
context-aware status pushes, and client-count updates.

## src/main/webui/server/__tests__/WebUIManager.test.ts

Tests for WebUIManager startup, port binding, static asset serving,
and integration with auth and websocket services.

## src/main/webui/server/__tests__/security-middleware.test.ts

Tests for WebUI security middleware header injection and
pass-through request handling.

## src/main/webui/server/__tests__/static-asset-options.test.ts

Tests for the WebUI static asset cache policy helpers and
Express static options factory.

## src/main/webui/server/api-routes.ts
...
## src/main/webui/server/auth-middleware.ts
...
Tests for camera WebUI routes covering proxy-config responses,
auth-sensitive behavior, and go2rtc-backed camera availability.

## src/main/webui/server/routes/__tests__/context-routes.test.ts

Tests for WebUI context routes that list available contexts
and switch the active printer context.

## src/main/webui/server/routes/__tests__/printer-status-routes.test.ts

Tests for printer-status WebUI routes covering normalized status
payloads and backend-availability guard rails.

## src/main/webui/server/routes/__tests__/route-helpers.test.ts

Tests for shared route helper utilities that resolve printer
contexts and emit standardized API error responses.

## src/main/webui/server/routes/__tests__/spoolman-routes.test.ts

Tests for Spoolman WebUI routes covering config, search,
selection, and active-spool state responses.

## src/main/webui/server/routes/__tests__/temperature-routes.test.ts

Jest coverage for the WebUI temperature-control route handlers,
...
## src/main/webui/server/routes/calibration-routes.ts

Calibration API route registrations for the WebUI server.
...
## src/main/webui/server/routes/context-routes.ts

Printer context management routes (list + switch active context).

## src/main/webui/server/routes/debug-routes.ts

Debug log routes for WebUI - provides endpoints for downloading debug and network logs.
...
## src/main/webui/server/routes/filtration-routes.ts

Filtration (AD5M Pro) control routes for the WebUI server.

## src/main/webui/server/routes/job-routes.ts

Job listing and control routes (local/recent files plus start job).

## src/main/webui/server/routes/printer-control-routes.ts
...
## src/main/webui/server/routes/route-helpers.ts
...
## src/main/webui/server/routes/spoolman-routes.ts

Spoolman integration routes (config, search, active spool management).

## src/main/webui/server/routes/temperature-routes.ts

Temperature control API routes for the WebUI server.

## src/main/webui/server/routes/test-server.ts

Lightweight Express test server helper used by route tests
to stand up isolated HTTP fixtures on random local ports.

## src/main/webui/server/routes/theme-routes.ts

WebUI theme configuration routes.

## src/main/webui/server/security-middleware.ts
...
## src/main/webui/server/static-asset-options.ts

Shared no-store cache options for serving built WebUI assets.
...
## src/main/webui/static/__tests__/app.test.ts

Jest coverage for the WebUI app bootstrap module.
...
## src/main/webui/static/core/AppState.ts
...
## src/main/webui/static/core/Transport.ts
...
## src/main/webui/static/core/__tests__/Transport.test.ts

Jest coverage for the WebUI transport layer.
...
## src/main/webui/static/features/__tests__/authentication.test.ts

Jest coverage for the WebUI authentication feature module.
...
Jest coverage for the WebUI camera feature helpers.
...
## src/main/webui/static/features/__tests__/context-switching.test.ts

Jest coverage for WebUI printer context switching.
...
## src/main/webui/static/features/__tests__/spoolman.test.ts

Jest coverage for the WebUI Spoolman feature module.
...
## src/main/webui/static/features/authentication.ts
...
Camera streaming helpers for the WebUI client using go2rtc.
...
## src/main/webui/static/features/context-switching.ts
...
## src/main/webui/static/features/job-control.ts
...
## src/main/webui/static/features/layout-theme.ts
...
## src/main/webui/static/features/material-matching.ts
...
## src/main/webui/static/features/material-station.ts

Material Station dashboard card + slot editor for the WebUI client.
...
## src/main/webui/static/features/spoolman.ts
...
## src/main/webui/static/grid/WebUIComponentRegistry.ts
...
## src/main/webui/static/grid/WebUIGridManager.ts
...
## src/main/webui/static/grid/WebUILayoutPersistence.ts
...
## src/main/webui/static/grid/WebUIMobileLayoutManager.ts
...
## src/main/webui/static/grid/types.ts
...
## src/main/webui/static/lib/video-rtc.js

Vendored `VideoRTC` custom element for WebUI camera playback.
...
## src/main/webui/static/shared/__tests__/formatting.test.ts

Jest coverage for shared WebUI formatting helpers.
...
## src/main/webui/static/shared/__tests__/icons.test.ts

Jest coverage for shared WebUI Lucide icon helpers.
...
## src/main/webui/static/shared/__tests__/palette.test.ts

Sanity coverage for the WebUI-local filament palette port,
guarding the per-model selection and nearest-match snapping used by the
material-station slot editor.

## src/main/webui/static/shared/dom.ts
...
## src/main/webui/static/shared/formatting.ts
...
## src/main/webui/static/shared/icons.ts
...
## src/main/webui/static/shared/palette.ts

Filament palette for the WebUI material-station slot editor.

The WebUI static client compiles standalone (rootDir = static dir) and is served
as raw ES modules, so it cannot import the main/renderer `@shared/palette`. This
is a faithful local port of `src/shared/palette-core.ts` + `ad5x-palette.ts` +
`creator5-palette.ts`: FlashForge printers only render a fixed set of materials
and colors on their material station, so arbitrary Spoolman values must be
...
## src/main/webui/static/ui/dialogs.ts
...
## src/main/webui/static/ui/header.ts
...
## src/main/webui/static/ui/panels.ts
...
## src/main/windows/dialogs/SpoolmanOfflineDialog.ts

Helper for creating and managing the Spoolman offline warning dialog.

## src/main/windows/factories/ComponentDialogWindowFactory.ts
...
## src/main/windows/factories/CoreWindowFactory.ts
...
## src/main/windows/factories/ShortcutConfigWindowFactory.ts
...
Preload script for secure renderer-to-main process IPC communication.
...
## src/renderer/src/lib/video-rtc.js

Vendored `VideoRTC` custom element for desktop renderer camera playback.
...
## src/renderer/src/renderer/gridController.ts
...
## src/renderer/src/renderer/logging.ts
...
## src/renderer/src/renderer/perPrinterStorage.ts

Helper utilities for per-printer layout and shortcut persistence.

## src/renderer/src/renderer/services/printer-state.ts

Renderer-side printer state tracker service.
...
## src/renderer/src/renderer/services/ui-updater.ts

Renderer-side UI updater service.
...
## src/renderer/src/renderer/shortcutButtons.ts
...
## src/renderer/src/renderer/utils/icons.ts
...
## src/renderer/src/ui/about-dialog/about-dialog-preload.ts

Preload script for the About dialog providing app/version metadata.
...
## src/renderer/src/ui/about-dialog/about-dialog-renderer.ts

Renderer controller for the About dialog showing app metadata and resource links.

## src/renderer/src/ui/auto-connect-choice/auto-connect-choice-preload.ts

Auto-connect choice dialog preload script - Provides secure IPC bridge
...
## src/renderer/src/ui/auto-connect-choice/auto-connect-choice-renderer.ts
...
## src/renderer/src/ui/calibration-dialog/calibration-dialog-preload.ts

Preload script for the Calibration Assistant dialog.
...
## src/renderer/src/ui/calibration-dialog/calibration-dialog-renderer.ts

Main renderer script for the Calibration Assistant dialog.
...
## src/renderer/src/ui/calibration/visualization/AnimatedRecommendationVisualizer.ts

Animated visualization for screw adjustments and tape recommendations.

@module renderer/ui/calibration/visualization/AnimatedRecommendationVisualizer

## src/renderer/src/ui/calibration/visualization/BedMeshVisualizer.ts

Canvas-based bed mesh visualization component.
...
## src/renderer/src/ui/calibration/visualization/ColorScales.ts
...
## src/renderer/src/ui/calibration/visualization/ShaperPlotVisualizer.ts

Canvas-based visualization for input shaper frequency analysis.
...
## src/renderer/src/ui/calibration/visualization/index.ts

Exports for the calibration visualization module.
Provides canvas-based visualization components for bed mesh heatmaps.

@module renderer/ui/calibration/visualization

## src/renderer/src/ui/component-dialog/component-dialog-preload.ts

Preload script for component dialog windows
...
## src/renderer/src/ui/component-dialog/component-dialog.ts
...
## src/renderer/src/ui/components/ComponentManager.ts
...
## src/renderer/src/ui/components/additional-info/additional-info.ts
...
## src/renderer/src/ui/components/additional-info/index.ts
...
## src/renderer/src/ui/components/base/component.ts
...
## src/renderer/src/ui/components/base/index.ts
...
## src/renderer/src/ui/components/base/types.ts
...
## src/renderer/src/ui/components/camera-preview/index.ts
...
## src/renderer/src/ui/components/controls-grid/controls-grid.ts
...
## src/renderer/src/ui/components/controls-grid/index.ts
...
## src/renderer/src/ui/components/creator5-temperature/creator5-temperature.ts

Creator 5 Temperature Card
...
## src/renderer/src/ui/components/creator5-temperature/index.ts

Creator 5 Temperature Card Exports

Central export point for the unified Creator 5 / 5 Pro temperature card
(settable 4 tools + bed + chamber).

@module ui/components/creator5-temperature

## src/renderer/src/ui/components/filtration-controls/filtration-controls.ts
...
## src/renderer/src/ui/components/filtration-controls/index.ts
...
## src/renderer/src/ui/components/index.ts
...
## src/renderer/src/ui/components/job-info/index.ts
...
## src/renderer/src/ui/components/job-info/job-info.ts
...
## src/renderer/src/ui/components/job-stats/index.ts
...
## src/renderer/src/ui/components/job-stats/job-stats.ts
...
## src/renderer/src/ui/components/log-panel/index.ts
...
## src/renderer/src/ui/components/log-panel/log-panel.ts
...
## src/renderer/src/ui/components/material-station/index.ts

Material Station Component Exports
...
## src/renderer/src/ui/components/material-station/material-station.ts
...
## src/renderer/src/ui/components/material-station/types.ts

Material Station Component Type Definitions
...
## src/renderer/src/ui/components/model-preview/index.ts
...
## src/renderer/src/ui/components/model-preview/model-preview.ts
...
## src/renderer/src/ui/components/printer-status/index.ts
...
## src/renderer/src/ui/components/printer-status/printer-status.ts
...
## src/renderer/src/ui/components/printer-tabs/PrinterTabsComponent.ts
...
## src/renderer/src/ui/components/printer-tabs/index.ts
...
## src/renderer/src/ui/components/spoolman/index.ts
...
## src/renderer/src/ui/components/spoolman/spoolman.ts
...
## src/renderer/src/ui/components/spoolman/types.ts
...
## src/renderer/src/ui/components/temperature-controls/index.ts
...
## src/renderer/src/ui/components/temperature-controls/temperature-controls.ts
...
## src/renderer/src/ui/components/tool-temps/index.ts

Tool Temperatures Component Exports

Central export point for the per-tool nozzle temperature grid component
(Creator 5 series).

@module ui/components/tool-temps

## src/renderer/src/ui/components/tool-temps/tool-temps.ts

Tool Temperatures Component
...
## src/renderer/src/ui/connect-choice-dialog/connect-choice-dialog-preload.ts

Connect choice dialog preload - Secure IPC bridge for the connect choice
dialog window. Exposes safe API for dialog interaction and result handling.

## src/renderer/src/ui/connect-choice-dialog/connect-choice-dialog-renderer.ts
...
## src/renderer/src/ui/file-manager/file-manager-preload.ts

Preload script for the SFTP printer file manager dialog.
...
## src/renderer/src/ui/file-manager/file-manager-renderer.ts

Renderer for the SFTP printer file manager dialog.
...
## src/renderer/src/ui/gridstack/ComponentRegistry.ts
...
## src/renderer/src/ui/gridstack/EditModeController.ts
...
## src/renderer/src/ui/gridstack/GridStackManager.ts
...
## src/renderer/src/ui/gridstack/LayoutPersistence.ts
...
## src/renderer/src/ui/gridstack/__tests__/defaults.migration.test.ts

Tests for legacy component-id migration in saved layouts.
...
## src/renderer/src/ui/gridstack/defaults.ts
...
## src/renderer/src/ui/gridstack/types.ts
...
## src/renderer/src/ui/input-dialog/input-dialog-preload.ts

Preload script for generic input dialog with secure IPC communication.
...
## src/renderer/src/ui/input-dialog/input-dialog-renderer.ts
...
## src/renderer/src/ui/job-picker/job-picker-preload.ts

Job Picker Dialog Preload Script

## src/renderer/src/ui/job-picker/job-picker-renderer.ts
...
## src/renderer/src/ui/job-uploader/job-uploader-preload.ts

Preload script for job uploader dialog with 3MF multi-color support.
...
## src/renderer/src/ui/job-uploader/job-uploader-renderer.ts
...
## src/renderer/src/ui/log-dialog/log-dialog-preload.ts

Log Dialog Preload Script

## src/renderer/src/ui/log-dialog/log-dialog-renderer.ts
...
## src/renderer/src/ui/material-info-dialog/material-info-dialog-preload.ts

Preload script for material information display dialog.
...
## src/renderer/src/ui/material-info-dialog/material-info-dialog-renderer.ts
...
## src/renderer/src/ui/material-matching-dialog/material-matching-dialog-preload.ts

Preload script for material matching dialog with IFS slot assignment.
...
## src/renderer/src/ui/material-matching-dialog/material-matching-dialog-renderer.ts
...
## src/renderer/src/ui/palette/palette-preload.ts

Preload script for Component Palette window
...
## src/renderer/src/ui/palette/palette.ts
...
## src/renderer/src/ui/printer-connected-warning/printer-connected-warning-preload.ts

Preload script for Printer Connected Warning Dialog
...
## src/renderer/src/ui/printer-connected-warning/printer-connected-warning-renderer.ts
...
## src/renderer/src/ui/printer-selection/printer-selection-preload.ts

Preload script for printer selection dialog with dual-mode support.
...
## src/renderer/src/ui/printer-selection/printer-selection-renderer.ts
...
## src/renderer/src/ui/send-cmds/send-cmds-preload.ts

Send Commands Dialog preload script providing secure IPC bridge for sending raw printer
...
## src/renderer/src/ui/send-cmds/send-cmds-renderer.ts
...
## src/renderer/src/ui/settings/sections/AutoUpdateSection.ts
...
## src/renderer/src/ui/settings/sections/DesktopThemeSection.ts
...
## src/renderer/src/ui/settings/sections/DiscordWebhookSection.ts

Handles the Discord webhook test controls.

## src/renderer/src/ui/settings/sections/InputDependencySection.ts
...
## src/renderer/src/ui/settings/sections/PrinterContextSection.ts

Handles UI updates tied to per-printer context availability.

## src/renderer/src/ui/settings/sections/RoundedUISection.ts

Handles Rounded UI capability detection + warnings.

## src/renderer/src/ui/settings/sections/SSHSection.ts

Settings section for centralized per-printer SSH credentials.
...
## src/renderer/src/ui/settings/sections/SettingsSection.ts
...
## src/renderer/src/ui/settings/sections/SpoolmanTestSection.ts

Handles the Spoolman connection test controls.

## src/renderer/src/ui/settings/sections/TabSection.ts
...
## src/renderer/src/ui/settings/settings-preload.ts

Settings Dialog preload script providing secure IPC bridges for both global
...
## src/renderer/src/ui/settings/types.ts
...
## src/renderer/src/ui/settings/types/external.ts

Shared API interfaces for the settings renderer sections.

## src/renderer/src/ui/shared/log-panel/LogPanelController.ts
...
## src/renderer/src/ui/shared/log-panel/index.ts

Shared log panel exports.

## src/renderer/src/ui/shared/lucide.ts
...
## src/renderer/src/ui/shared/theme-utils.ts
...
## src/renderer/src/ui/shell/RebootController.ts

Overlay controller for the remote "Reboot Printer" lifecycle.

Claims the shared #loading-overlay exclusively for a reboot and drives it
through its phases: rebooting -> reconnecting -> success (auto-dismiss) or
timeout / failed (Retry / Close). Transitions come from main-process
'printer:reboot-status' events; a renderer-side 150s safety timer covers the
case where those events never arrive.
...
## src/renderer/src/ui/shell/ShellController.ts

Shell controller for the renderer's window chrome.
...
## src/renderer/src/ui/shortcut-config-dialog/shortcut-config-dialog-preload.ts

Preload script for shortcut configuration dialog
...
## src/renderer/src/ui/shortcut-config-dialog/shortcut-config-dialog.ts
...
## src/renderer/src/ui/shortcuts/ShortcutConfigManager.ts
...
## src/renderer/src/ui/shortcuts/types.ts
...
## src/renderer/src/ui/single-color-confirmation-dialog/single-color-confirmation-dialog-preload.ts

Single Color Confirmation Dialog preload script providing secure IPC bridge
...
## src/renderer/src/ui/single-color-confirmation-dialog/single-color-confirmation-dialog-renderer.ts
...
## src/renderer/src/ui/spoolman-dialog/spoolman-dialog-preload.ts

Spoolman Dialog Preload Script
...
## src/renderer/src/ui/spoolman-dialog/spoolman-dialog-renderer.ts
...
## src/renderer/src/ui/spoolman-offline-dialog/spoolman-offline-dialog-preload.ts

Preload script for Spoolman offline dialog.

## src/renderer/src/ui/spoolman-offline-dialog/spoolman-offline-dialog-renderer.ts

Renderer logic for Spoolman offline dialog.

## src/renderer/src/ui/status-dialog/status-dialog-preload.ts

Status Dialog preload script exposing secure IPC bridge for comprehensive
...
## src/renderer/src/ui/status-dialog/status-dialog-renderer.ts
...
## src/renderer/src/ui/update-available/update-available-preload.ts

Preload script for the auto-update dialog exposing safe IPC bridges.
...
## src/renderer/src/ui/update-available/update-available-renderer.ts
...
## src/scripts/__tests__/copy-webui-assets.test.ts

Jest coverage for the WebUI asset copy/build-stamp script.
...
Tests for the filament palette base class and per-model palettes.
...
## src/shared/ad5x-palette.ts

AD5X IFS material/color palette (firmware-confirmed).
...
Shared component definitions data
...
## src/shared/creator5-palette.ts

Creator 5 / Creator 5 Pro material/color palette (firmware-confirmed).
...
## src/shared/palette-core.ts

Filament palette base class and shared color-science helpers.
...
Filament palette entry point — model → palette selection.
...
## src/shared/themeColorUtils.ts

Shared theme color utilities for deriving runtime CSS variables.

## src/shared/types/PrinterContext.ts
...
## src/shared/types/calibration.ts

Shared type definitions for the Calibration Assistant module.
...
## src/shared/types/camera/camera.types.ts

Type definitions for camera streaming system using go2rtc.
...
## src/shared/types/camera/index.ts
...
## src/shared/types/file-manager.ts

Shared type definitions for the SFTP-based printer file manager.
...
## src/shared/types/global-main.d.ts
...
## src/shared/types/lucide-icons.d.ts

Ambient declarations for Lucide ESM internals used by the app.
...
## src/shared/types/printer-backend/backend-operations.ts
...
## src/shared/types/printer-backend/index.ts
...
## src/shared/types/printer-backend/printer-features.ts
...
## src/shared/types/printer-power.ts

Shared IPC types for the remote "Reboot Printer" feature.
...
## src/shared/types/shortcut-config.ts
...
## src/shared/types/ssh-settings.ts

Shared type definitions for centralized per-printer SSH settings.
...
## src/shared/types/web-api.types.ts

Shared WebUI API and WebSocket contract types.
...
## src/shared/utils/__tests__/printerSettingsDefaults.test.ts

Jest coverage for per-printer settings default helpers.
...
Centralized per-printer settings defaults and utilities.
...
Shared time formatting utilities for consistent time display across
