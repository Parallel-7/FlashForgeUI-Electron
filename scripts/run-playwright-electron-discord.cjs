 ...
 * Electron Discord notification validation against a real printer.
 *
 * Separate from the main hardware track (run-playwright-electron-hardware.cjs): this
 * one drives a single opt-in spec that posts to a live Discord webhook, so it stays
 * behind explicit IP/check-code environment variables rather than the shared
 * hardware-printers config.
 */
