/**
 * @fileoverview WindowManager is the singleton registry for the app's
 * BrowserWindow instances. Get it with getWindowManager(); the class itself is
 * not exported. Window factories register, look up, and clear window
 * references here, keyed by WindowType, through typed get/set/has accessors.
 *
 * A destroyed window counts as absent: hasWindow(), getActiveWindows(), and
 * the closeAll* methods skip destroyed windows, so callers never receive a
 * dead reference. Clear a slot by setting it to null when its window closes.
 */
