/**
 * @fileoverview A browser-safe, dependency-free EventEmitter with typed
 * events: pass an event map interface, and listener and emit arguments are
 * checked at compile time. Listener exceptions are caught and logged, so one
 * bad listener cannot break the others. emit iterates a copy of the listener
 * list, so a listener can safely add or remove listeners.
 */
